...
public class ScoreboardFileParser extends FileParser {
    ...
    public boolean read(Object game, MultipartFile file, S3Service s3) throws IOException {
        try{
            ...
            String s3Content = IOUtils.toString(s3.getFile(file.getOriginalFilename()).getObjectContent(), StandardCharsets.UTF_8).strip();
            ...
            multiplayerGame.getAttackerCaptain().setUsername(players[0]);
            multiplayerGame.getDefenderCaptain().setUsername(players[1]);
            ...
            for (int i = 4; i < lines.length; i++) {
                ...
                MultiplayerUserStats stats = MultiplayerUserStats.builder()
            ...
            StringBuilder continut =
                    new StringBuilder(
                            multiplayerGame.getGameId()
                                    + "\n"
                                    + multiplayerGame.getAttackerCaptain().getUsername()
                                    + " vs "
                                    + multiplayerGame.getDefenderCaptain().getUsername()
                                    + "\n");
    ...
    private void sortUserStats(MultiplayerGame multiplayerGame) {
        Map<String, MultiplayerUserStats> sortedUserStats = multiplayerGame
                .getUserStats()
                .entrySet()
                .stream()
                .sorted(
                        (e1, e2) -> {
                            if (e1.getValue().getKills()*2+e1.getValue().getHeadshots()*1.5 != e2.getValue().getKills()*2+e2.getValue().getHeadshots()*1.5) {
                                return (int) (e2.getValue().getKills()*2+e2.getValue().getHeadshots()*1.5 - e1.getValue().getKills()*2+e1.getValue().getHeadshots()*1.5);
                            }
                            if (e1.getValue().getKills() != e2.getValue().getKills()) {
                                return e2.getValue().getKills() - e1.getValue().getKills();
                            }
                            if (e1.getValue().getHeadshots() != e2.getValue().getHeadshots()) {
                                return e2.getValue().getHeadshots() - e1.getValue().getHeadshots();
                            }
